...
namespace RCCom.EditorTools
{
    ...
    public static class OperatorAssetValidator
    {
        ...
        {
            string path = AssetDatabase.GetAssetPath(roster);
            if (roster.unitIds == null || roster.unitIds.Count == 0)
            {
            ...
            foreach (string unitId in roster.unitIds)
            {
                ...
                {
                    errors.Add($"AllyUnitRoster에 빈 unitId가 있습니다: {path}");
                    continue;
                }

                if (!unitIds.Add(unitId))
                {
                    errors.Add($"한 AllyUnitRoster 안에서 unitId가 중복됩니다: {unitId} ({path})");
                    continue;
                }

                string definitionPath = AllyUnitCatalogBuilder.GetDefinitionPath(unitId);
                ...
                if (!string.Equals(definition.data.unitId, unitId, StringComparison.Ordinal))
                {
                    errors.Add($"Roster ID와 Definition ID가 다릅니다: {unitId} ({definitionPath})");
                }
        ...
        private static void ValidateAllyUnitAssets(List<string> errors, List<string> warnings)
        {
            ...
            {
                string unitId = definition.data != null ? definition.data.unitId : null;
                if (string.IsNullOrWhiteSpace(unitId) || !registeredIds.Contains(unitId))
                {
