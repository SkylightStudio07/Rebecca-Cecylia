...
## 5. 파일럿(PlayerData) 공통 스탯 강화 — 폐기됨

> **이 절은 파기되었다.** 조작 캐릭터(`PlayerData`) 자체를 대상으로 한 레벨형 선형 강화안 대신,
> "파츠를 갈아 끼우는" 장비형 강화 시스템으로 전면 대체한다. 상세 설계는
> [`플레이어_기체_파츠_시스템_설계안.md`](./플레이어_기체_파츠_시스템_설계안.md) 참고.
> 오퍼레이터 전용 트랙(§4, 이 문서)은 그대로 유지되며 새 파츠 시스템과 공존한다.
