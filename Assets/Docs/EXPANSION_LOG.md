...
## 2026-08-22 — 오퍼레이터 관리 카드 프리팹 수동 편집 보존

### 맥락
- 실제 초상화의 시각 중심과 텍스트 간격은 화면을 보면서 손으로 미세 조정하는 편이 빠르지만, 기존 `OperatorManagementSetup.Build()`는 실행할 때마다 `OperatorManagementCard.prefab`을 새로 생성해 수동 오프셋을 덮어썼다.

### 결정
- 관리 화면 빌더는 카드 프리팹이 이미 존재하면 그대로 로드해 사용하고, 없을 때만 기본 프리팹을 최초 생성한다.
- 기존 프리팹은 사람이 조정하는 UI 원본으로 취급하고, 재빌드 시에는 씬의 `RuntimeContent`와 Controller 배선만 갱신한다.
- 프리팹을 그대로 재사용하되 런타임에 필요한 Button, 상태 Image, 초상화, TMP, 상태별 Sprite 참조가 비어 있으면 검증 단계에서 즉시 실패하도록 했다.
