...
## 2026-08-23 — 오퍼레이터 관리 Deploy 버튼 호버 복구

### 증상과 원인
- Deploy의 `_1` Highlighted Sprite 연결은 정상이었지만, 활성 오퍼레이터나 잠긴 항목을 보고 있을 때 `Button.interactable = false`가 되어 Selectable이 Disabled 상태에 고정됐다. 따라서 포인터가 올라와도 Highlighted 상태로 전환되지 않았다.

### 결정
- Deploy 버튼은 콘텐츠 로딩 중에만 비활성화하고, 해금 여부와 이미 활성 상태인지에 대한 실행 가능 판정은 `Deploy()`에서 처리한다. 포인터 Hover 표현은 유지하면서 허용되지 않은 클릭은 아무 동작도 하지 않는다.
- SpriteSwap은 Normal=`_0`, Highlighted/Pressed=`_1`, Selected/Disabled=`_0`으로 명시해 키보드 포커스와 포인터 Hover를 분리한다.

### 의도적으로 하지 않은 것
- 별도 포인터 이벤트 컴포넌트나 Hover 오버레이는 추가하지 않았다. 기존 uGUI Button의 SpriteSwap 계약만 사용한다.
